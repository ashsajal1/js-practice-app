...
export default function QuizCard({ quiz, question }: { quiz: QuizProps, question: string }) {
    ...
    if (hasSubmitted) {
        return (<>
            <div className={`p-4 rounded border shadow ${isRightAnswer ? 'bg-green-200' : 'bg-red-200'}`}>
                ...
                {quiz.options.map((i) => (
                    <>
                        <p className={cn(`p-2 text-black border rounded mb-2 cursor-pointer select-none ${quiz.answer === i ? 'bg-green-600' : ''} ${selectedOption === quiz.answer && selectedOption === i ? 'bg-green-600' : ''} ${isRightAnswer && selectedOption !== i ? 'border border-black text-black dark:border-gray-800' : ''} ${!isRightAnswer && selectedOption === i ? 'bg-red-600' : ''} ${!isRightAnswer && selectedOption !== i ? 'border border-white text-black dark:border-gray-800' : ''}`)}>{i}</p>
                    </>
                ))}
    ...
    return (
        <div className="p-4 rounded border shadow dark:border-gray-600">
            ...
            {quiz.options.map((i) => (
                <>
                    <p onClick={() => setSelectedOption(i)} className={cn(`p-2 border rounded mb-2 cursor-pointer select-none dark:border-gray-800 dark:text-white ${selectedOption === i ? 'border-blue-700 dark:border-blue-800' : ''}`)}>{i}</p>
                </>
            ))}
